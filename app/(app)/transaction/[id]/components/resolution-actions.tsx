...
export function ResolutionActions({ status, txId }: ResolutionActionsProps) {
  const statusLabel = status === "Blocked" ? "diblokir" : status === "Held" ? "ditahan" : "disetujui"
    ...
    <div className="sticky bottom-0 border-t border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/80 px-6 py-3">
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex-1">
          <p className="text-xs font-semibold text-foreground">Aksi Penyelesaian</p>
          <p className="text-[11px] text-muted-foreground">{txId} sedang {statusLabel}; pilih langkah berikutnya</p>
        </div>
